...
```http
    ...
    example： https://api.bingbon.com/api/v1/market/trade/BTC_USDT?maxCount=0
```
...
| ------------- |----|----|----|
| symbol | String | 是 | 币对, 如 BTC_USDT |
| maxCount | Integer | 是 | 深度档位 默认是10, 目前最大值支持50 |
...
```http
    ...
    example： https://api.bingbon.com/api/v1/market/kline/BTC_USDT?period={period}&pagingSize={pagingSize}&startId={startId}
```
...
| ------------- |----|----|----|
| symbol | String | 是 | 币对, 如 BTC_USDT |
| period | Integer | 是 | K线类型,1min, 5min, 15min, 30min, 60min, 4hour, 1day, 1sec |
| pagingSize | Integer | 是 | 每页数量 |
| startId | String | 否 | 分页起始id，结果为小于此值，如:2018-09-20T00:00:00.000+0800 如果是轮询，第一次pagingSize，startId都不传，第二次startId传递返回的最后一个statDate |
